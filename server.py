...
class Server():
    ...
    def check_connection(self):
        ...
        try:
            if self.connection == "http":
                ...
                msg = f"{self.name} is up. On port {self.port} with \
                {self.connection}"
                success = True
                self.alert = False
            elif self.connection == "ssl":
                ssl.wrap_socket(socket.create_connection((self.name,
            ...
            msg = f"server: {self.name} timepout. On port {self.port}"
        except (ConnectionRefusedError, ConnectionResetError) as e:
            msg = f"server: {self.name} {e}"
        except Exception as e:
            msg = f"Not sure what happened... {e}"
    ...
    def ping(self):
        try:
            output = subprocess.check_output("ping - {} 1 {}".format('n' if \
                platform.system().lower() == "Linux" else 'c', self.name), \
